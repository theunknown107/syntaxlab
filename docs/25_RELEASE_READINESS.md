...
| Check | State |
|---|---|
| Production headers served from the real `_headers` | **PASS** — `npm run serve:prod`, asserted by test |
...
| **Real Cloudflare Pages preview deployment** | **NOT RUN — no credentials.** No `CLOUDFLARE_API_TOKEN`, no `wrangler.toml`, no linked project. The most production-faithful local server available was used instead and is asserted by test. **This is M13's, and is the third open gate.** |
...
|---|---|---|
| **Real preview deployment** | NOT RUN — no credentials in this environment | M13: deploy to a Cloudflare Pages preview and repeat the offline, CSP and update checks against it |
