...
| B3′ | User JSON → the DOM | Keys and values are entirely attacker-controlled | **Verified at M6.** Every key, value, path, error message and finding renders as a React text child; there is no HTML string in the path and the lint ban on raw-HTML props still holds. `__proto__` renders as a key like any other. E2E drives script, image and `javascript:` payloads through keys, values, errors and search results. |
...
| AC12 | A wrong cron explanation causes a production misconfiguration | Real-world operational damage | **Live from M14.** A golden corpus of 59 hand-written expressions, 8 of them valid in other schedulers and each asserted to be refused by name; refusal to parse 6/7 fields; timezone union of two members, enforced on the wire; every explanation carries a timezone section; OR-rule warning always emitted. One real instance was found and fixed during M14: `5/10` expanded to a set no scheduler produces. | **RR-09** |
