...
## 8. Commit authorship — a published fact, not a local detail

**Every commit is authored and committed as `SyntaxLab
<theunknown107@users.noreply.github.com>`.** That address is GitHub's noreply
form for this account: GitHub attributes commits using it to the account, and
it keeps a personal mailbox out of a public history that anyone can clone.

Two rules, both learned the expensive way.

**No personal email in a public commit.** The repository's `user.email` must be
the noreply address. If it is not, `git commit` will quietly use whatever the
global configuration says, and two commits reached the public history — and
Vercel's deployment metadata — carrying a personal Gmail before anyone noticed.
Check it, don't assume it:

```bash
git config user.email    # must be theunknown107@users.noreply.github.com
```

**No AI co-author trailers.** This is a solo repository, and

```
Co-Authored-By: <an AI assistant>
```

does not belong in its history. GitHub never listed a second contributor —
the address maps to no account — but 102 published commits carried the trailer
before it was removed. Nothing generated by a tool is co-authored by it in the
sense a trailer claims.

Both were corrected in a single history rewrite before the M14 push, and the
rewrite is not to be repeated: see §8.1.

### 8.1 History rewriting

**The public history is stable.** Rewriting it invalidates every clone and
every deployment reference, so it happens only for a genuine publication or
security problem — a leaked credential, a published personal identifier — and
never for tidiness.
...
| | |
|---|---|
| Back up first | A local ref (`refs/backup/<what>`), never pushed |
| Prefer | `git-filter-repo`, which is what git's own documentation recommends. `git filter-branch` works and was used here only because `filter-repo` was not installed |
| Change | Authorship metadata and trailers only |
| Preserve | Commit order, contents, ancestry, dates, and message text |
| Verify | **Tree hash of the tip, before and after.** Identical hashes prove the content did not move. A commit SHA changing is expected; a tree hash changing means something was edited |
| Push with | `git push --force-with-lease`, never `--force` — the lease is what refuses to overwrite a remote commit you have not seen |
