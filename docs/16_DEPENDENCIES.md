...
| The custom cron engine proves unreliable | Adopt `cron-parser` (~15 KB) for schedule computation; keep our own field parser for the explanation and positions. **Not triggered at M14:** 179 tests including 1 200-run property tests and a golden corpus of 59 hand-written expressions, with no correctness problem that a library would have prevented. The one semantic defect found — `5/10` — was ours to fix and is now pinned. |
